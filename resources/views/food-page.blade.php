...
<div class="category-page-wrapper">
    <div class="container my-5">
        ...
        <div class="text-center mb-5">
            ...
            <h1 class="mt-3">{{ $category }} কালেকশন</h1>
            <p>আমাদের স্পেশাল {{ $category }} গুলো দেখুন</p>
        </div>
            ...
            @forelse($foods as $food)
                <div class="col-md-6">
                    <div class="card food-card">
                        <img src="{{ asset($food->image) }}" class="food-image" alt="{{ $food->name }}">
                        <div class="card-body">
                            <h3>{{ $food->name }}</h3>
                            <p class="text-muted">{{ $food->details }}</p>
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                @auth
                                    @if(auth()->user()->role == 'user')
                                        <button class="btn btn-primary" onclick="openOrderModal('{{ $food->name }}', '{{ $food->price }}')">
                    ...
                    <div class="alert alert-info text-center">
                        ...
                        @auth
                            @if(auth()->user()->role == 'admin')
                                <a href="{{ route('admin.foods') }}" class="btn btn-primary">খাবার যোগ করুন</a>
                            @endif
                        @endauth
                    </div>
                </div>
